...
| `DB` | 否 | Cloudflare D1 数据库绑定名，绑定名必须是 `DB`。**数据库版数据层**：黑名单、/add_ad_admin 白名单、用户记录（资料/最后对话时间/活跃群组/消息计数）均存 D1；未绑定 D1 时自动回退 KV 模式。首次启用时会自动建表并把 KV 旧数据导入一次（幂等）。 |
...
| `active_group_ids` | TEXT | 活跃群组 ID 数组（JSON，如 `["-1001","-1002"]`） |
