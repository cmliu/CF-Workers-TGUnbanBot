...
- **多主群完全对等**：`GROUP_ID` 支持英文逗号分隔配置多个主群（如 `-1001,-1002,-1003`），解析为规范化 chatId 数组 `GROUP_ID_SET` 后，**数组内所有主群完全对等**（单个值不带逗号时行为与旧版完全一致）。管理员豁免 = **任一主群管理员**；`/ad`、`/ban`、`/spam`、`/unban`、入群封禁级拦截、消息路径查杀、自助解封均对所有主群生效；系统后台通知（如自助解封上报、GKY 二次审核上报）**广播到全部主群**；`/ad` 投票"动作发生在哪个主群就只在哪个主群处理"，不跨群广播。
