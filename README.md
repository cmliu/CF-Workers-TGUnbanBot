...
- **联网黑名单自动查杀**（bot 为群管理员且具备"限制成员"权限的群聊，含 `GROUP_ID` 主群的消息路径）：真人成员发送消息时自动比对联网黑名单，命中即禁言。新 TGID 自动建档（`active_group_ids` 记录本群 + 状态"健康"）；存量用户在本群状态为"健康"且命中联网黑名单（`is_blacklisted=1`）时自动禁言，同步本群状态为"禁言"，并在群内通知。非 `GROUP_ID` 群的通知附"🔨 永久封禁 / ✅ 加入白名单"按钮（仅本群管理员可点）：点"永久封禁"→ 踢出 + 本群状态"封禁"；点"加入白名单"→ 解除封禁/禁言 + 本群状态"白名单"（本群豁免，不再自动查杀）。`GROUP_ID` 主群同样纳入消息路径查杀（主群内 `/spam`、`/ban` 只拉黑而群内状态仍为"健康"的用户再次发言即被自动兜底），但命中通知**不带按钮**，文案提示管理员用 `/ban`（永久封禁）/ `/unban`（移出联网黑名单）命令处置；`GROUP_ID` 主群入群仍走传统封禁级拦截，私聊不触发查杀。
...
| `禁言` | 被禁发消息（restrictChatMember） | `/ban` 在群内禁言成功；`GROUP_ID` 群 `/spam` 拉黑后即时禁言；`/ad` 投票通过且用户在群内被禁言；联网查杀命中联网黑名单禁言（含 `GROUP_ID` 主群消息路径）；非 GROUP_ID 群 `/spam` |
