...
- 非主群内（bot 为该群管理员时），管理员同样可使用 `/ban`、`/spam`、`/unban`：`/ban` 标记本群状态"封禁"并封禁踢出；`/spam`（回复）标记本群状态"禁言"并禁言；`/unban` 标记本群状态"白名单"并解除封禁/禁言。这三个命令在非主群内**不会**修改 `is_blacklisted`（联网黑名单）；联网黑名单只能由主群(任一)内的 `/ad`、`/ban`、`/spam`、`/unban` 修改。
...
4. 机器人会**逐主群**检查用户在所有主群（`GROUP_ID` 多主群数组）内的状态，并在各群执行恢复：
...
| `/ban 用户ID` / 回复 `/ban` | 非主群（bot 为该群管理员） | 标记本群状态"封禁"并封禁踢出，**不修改联网黑名单**（`is_blacklisted`）。 |
| `/spam`（回复） | 非主群（bot 为该群管理员） | 本群禁言 + 状态"禁言"，**不修改联网黑名单**。 |
