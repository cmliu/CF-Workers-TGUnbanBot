...
- 管理员私聊或在被管理群组内发送 `/ban 用户ID`：将用户加入联网黑名单（数据库版存 D1，未绑定 D1 时自动回退 KV）；在被管理群组内使用时会同步禁言该用户。
- 管理员私聊或在被管理群组内发送 `/unban 用户ID`：将用户移出联网黑名单，并同步解除其在被管理群组里的封禁或禁言。
- 管理员在被管理群组内回复用户消息发送 `/ban` 或 `/unban`：对被回复用户执行对应的联网黑名单操作；`/ban` 会同步禁言该用户，`/unban` 会同步解除封禁或禁言。
- 管理员在群内回复用户消息发送 `/spam`：将被回复用户加入联网黑名单。
- 管理员发送 `/check 用户ID`，或在群内回复用户消息发送 `/check`：查询用户的 GKY 封禁记录与联网黑名单状态（数据库版附带封禁原因与时间），并返回二次审核操作提示。
- `/ad`（隐藏命令，不出现在命令菜单）：群内回复某条消息发起举报投票，把被举报消息内容与**被举报人的昵称/用户名/简介**一起交给 Workers AI（默认 `@cf/openai/gpt-oss-20b`，可用 `AD_AI_MODEL` 更换）按群规判断威胁评级（广告可能把内容藏匿于昵称简介中，故一并纳入判断），评级决定本次投票生效阈值（赞成票数）：🔴 S 极危 2 票、🟠 A 高危 3 票、🟡 B 危险 4 票、🟢 C 可疑 5 票、🔵 D 低危 6 票、⚪ E 无害 8 票；**反对票阈值 = 10 − 赞成票阈值**（S 反 8 票、A 反 7 票、B 反 6 票、C 反 5 票、D 反 4 票、E 反 2 票），任一边先到即结束；发起人自己点『反对』即视为放弃举报，投票立即关闭（不执行任何处理）。AI 有响应但无法识别/拒绝答复（如安全策略拒答、格式错乱）时按 🟡 B 危险（4 票）处理；无消息内容且未获取到用户资料、或 AI 调用失败时按 🟢 C 可疑（5 票）处理。群规可通过 `AD_GROUP_RULES` 环境变量自定义。投票通过后，为避免广告号显示名称（含 @用户名）中的广告内容二次传播，被举报人名称将自动脱敏显示（仅保留首尾字符）。**普通用户**在群内回复消息发送 `/ad` 也可发起举报：内容交 AI 评级，仅 S/A/B（极危/高危/危险）评级会弹出投票，C/D/E 或 AI 不可用时完全静默；群组**助推者**（Telegram Boost）或 `/add_ad_admin` 白名单成员可与管理员一样直接发起投票。
...
- **多主群完全对等**：`GROUP_ID` 支持英文逗号分隔配置多个主群（如 `-1001,-1002,-1003`），解析为规范化 chatId 数组 `GROUP_ID_SET` 后，**数组内所有主群完全对等**（单个值不带逗号时行为与旧版完全一致）。管理员豁免 = **任一主群管理员**；`/ad`、`/ban`、`/spam`、`/unban`、入群封禁级拦截、消息路径查杀、自助解封均对所有主群生效；自助解封恢复明细系统通知按群定向发送（仅向发生真实动作/失败的主群推送本群明细）；GKY 二次审核上报仍会广播到全部主群；`/ad` 投票"动作发生在哪个主群就只在哪个主群处理"，不跨群广播。
- **联网黑名单自动查杀**（bot 为群管理员且具备"限制成员"权限的群聊，含主群(任一)的消息路径）：真人成员发送消息时自动比对联网黑名单，命中即禁言。新 TGID 自动建档（`active_group_ids` 记录本群 + 状态"健康"）；存量用户在本群状态为"健康"且命中联网黑名单（`is_blacklisted=1`）时自动禁言，同步本群状态为"禁言"，并在群内通知。非主群的通知附"🔨 永久封禁 / ✅ 加入白名单"按钮（仅本群管理员可点）：点"永久封禁"→ 踢出 + 本群状态"封禁"；点"加入白名单"→ 解除封禁/禁言 + 本群状态"白名单"（本群豁免，不再自动查杀）。主群(任一)同样纳入消息路径查杀（主群内 `/spam`、`/ban` 只拉黑而群内状态仍为"健康"的用户再次发言即被自动兜底），但命中通知**不带按钮**，文案提示管理员用 `/ban`（永久封禁）/ `/unban`（移出联网黑名单）命令处置；主群(任一)入群仍走传统封禁级拦截，私聊不触发查杀。
- 非主群内（bot 为该群管理员时），管理员同样可使用 `/ban`、`/spam`、`/unban`：`/ban` 标记本地黑名单"封禁"并封禁踢出；`/spam`（回复）标记本地黑名单"禁言"并禁言；`/unban` 标记本群状态"白名单"并解除封禁/禁言。这三个命令在非主群内**不会**修改 `is_blacklisted`（联网黑名单）；联网黑名单只能由主群(任一)内的 `/ad`、`/ban`、`/spam`、`/unban` 修改。
...
| `GROUP_ID` | 是 | 目标 Telegram 群组 ID，一般是负数，例如 `-1001234567890`。**支持多个**：用英文逗号分隔（如 `-1001,-1002,-1003`，会逐段 trim、去空、去重），配置的所有群组被完全对等视为"主群"；单个值不带逗号时行为与旧版一致。解析后为空集合（如缺失或纯逗号/空白输入）会启动即报错。 |
...
| `AD_VOTE_THRESHOLD` | 否 | `/ad` 举报投票在无可判断内容（无消息且未取到用户资料）/ AI 调用失败时的回退赞成票数。默认 5（即 🟢C 可疑）。AI 评级生效时，赞成票数由评级决定（S=2 / A=3 / B=4 / C=5 / D=6 / E=8），反对票数恒为 `10 − 赞成票数`（S=8 / A=7 / B=6 / C=5 / D=4 / E=2），该变量不参与；AI 有响应但无法识别/拒答时固定按 🟡B 危险 4 票（反对 6 票），也不受该变量影响。 |
...
KV 中会使用 `blacklist` 这个 key 保存联网黑名单数组（数据库版下仅作迁移源，迁移完成后不再读写）。`ad_vote:<token>` 保存 `/ad` 举报投票状态（7 天 TTL，始终存 KV，利用 TTL 自动过期）。
...
| `is_gky_blacklisted` | INTEGER | 是否在 GKY 联网黑名单（/check 查询 GKY 有封禁记录时自动标记，0/1，后期备用） |
...
| `last_self_unban_at` | INTEGER | 自助解封口令触发时间（epoch 秒；0 = 从未触发）。10 分钟内重复发送口令将被冷却拒绝（防刷，详见「用户自助解封」） |
...
| `禁言` | 被禁发消息（restrictChatMember） | `/ban` 在群内禁言成功；主群 `/spam` 拉黑后即时禁言；`/ad` 投票通过且用户在群内被禁言；联网查杀命中联网黑名单禁言（含主群(任一)消息路径）；非主群 `/spam` |
| `封禁` | 被踢出群组（banChatMember） | 黑名单用户入群被拦截封禁；`/ad` 投票通过且用户不在群内被永久封禁；联网查杀按钮"永久封禁"；非主群 `/ban` |
| `白名单` | 被本群管理员显式豁免，不再触发联网查杀 | 联网查杀通知点"加入白名单"；非主群 `/unban` |
| `管理员` | 群主/管理员（豁免查杀） | 管理员发言自动标记（任一主群管理员） |
...
如果要使用 `/ban`、`/unban 用户ID`、`/spam` 这些联网黑名单功能，需要创建 KV（**推荐同时创建 D1，见 2.1；绑定 D1 后 KV 仅作一次性迁移源**）：
...
如果不绑定 KV，自助解封和 GKY 封禁查询仍可运行，但联网黑名单相关命令会提示未绑定存储。
...
在 Cloudflare Dashboard 中确保该 Worker 已绑定名为 `AI` 的 Workers AI 资源（也可以直接在 `wrangler.toml` 保留 `[ai]` 配置后重新 `wrangler deploy`）。若未绑定或调用失败，`/ad` 会自动回退为 🟢C 可疑（5 票）；若 AI 有响应但内容无法识别或拒绝答复（如触发安全策略），则按 🟡B 危险（4 票）处理，均不影响投票功能本身。
...
4. 触发时间会记入数据库（`last_self_unban_at`，未绑定 D1 时回退 KV 键 `selfunban_cd:<tgid>`）：**10 分钟内**再次发送口令时，机器人不再处理解封、也不向主群广播，仅回复"由于您近期已使用过自助解封，将不再处理您的自助解封"，并附上可再次使用的时间——Telegram 不提供用户时区信息，该时间以 UTC 标准时间 + 东八区（北京时间）双时区展示。

5. 机器人会**逐主群**检查用户在所有主群（`GROUP_ID` 多主群数组）内的状态，并在各群执行恢复：
   - 如果用户在某主群被封禁，调用 `unbanChatMember` 解封。
   - 如果用户在某主群被禁言，调用 `restrictChatMember` 恢复发言权限。
   - 处理结果作为系统后台通知**广播到所有主群**；如果用户存在 GKY 封禁记录，会在所有主群提醒管理员进行二次审核。

### 管理员命令

| 命令 | 使用位置 | 说明 |
| --- | --- | --- |
| `/ban 用户ID` | 私聊机器人或被管理群组 | 将用户加入联网黑名单（D1 优先，未绑定回退 KV）；在被管理群组内使用时同步禁言该用户。 |
| `/unban 用户ID` | 私聊机器人或被管理群组 | 将用户移出联网黑名单，并同步解除其在被管理群组里的封禁或禁言。 |
| 回复用户消息发送 `/ban` | 被管理群组 | 将被回复用户加入联网黑名单，并同步禁言该用户。 |
| 回复用户消息发送 `/unban` | 被管理群组 | 将被回复用户移出联网黑名单，并同步解除其封禁或禁言。 |
| `/spam` | 在群内回复某条消息 | 将被回复消息的发送者加入联网黑名单。 |
...
| `/ban 用户ID` / 回复 `/ban` | 非主群（bot 为该群管理员） | 标记本地黑名单"封禁"并封禁踢出，**不修改联网黑名单**（`is_blacklisted`）。 |
| `/spam`（回复） | 非主群（bot 为该群管理员） | 本地黑名单"禁言"并禁言，**不修改联网黑名单**。 |
| `/unban 用户ID` / 回复 `/unban` | 非主群（bot 为该群管理员） | 加入本群白名单并解除封禁/禁言，**不修改联网黑名单**。 |
...
- `GROUP_ID` 是机器人所在的目标群组 ID，可配置多个（英文逗号分隔），所有配置群组完全对等视为主群。
